...
static const char mconf_readme[] = N_(
	...
	"Legend: [*] built-in  [ ] excluded  <M> module  < > module capable"), radiolist_instructions[] = N_(
	"Use the arrow keys to navigate this window or "
	"press the hotkey of the item you wish to select "
	"followed by the <SPACE BAR>. "
	"Press <?> for additional information about this option."), inputbox_instructions_int[] = N_(
	"Please enter a decimal value. "
	"Fractions will not be accepted.  "
	"Use the <TAB> key to move from the input field to the buttons below it."), inputbox_instructions_hex[] = N_(
	"Please enter a hexadecimal value. "
	"Use the <TAB> key to move from the input field to the buttons below it."), inputbox_instructions_string[] = N_(
	"Please enter a string value. "
	"Use the <TAB> key to move from the input field to the buttons below it."), setmod_text[] = N_(
	"This feature depends on another which has been configured as a module.\n"
	"As a result, this feature will be built as a module."), nohelp_text[] = N_(
...
//	sym_calc_value (sym);
//	size = snprintf (menu_backtitle, sizeof(menu_backtitle), _("%s - Linux Kernel v%s Configuration"), config_filename, sym_get_string_value (sym));
...
//	sym = sym_lookup ("KERNELVERSION", 0);
//	sym_calc_value (sym);
	size = snprintf (menu_backtitle, sizeof(menu_backtitle), _("%s - Menu Configuration Interface"), config_filename);
	if (size >= sizeof(menu_backtitle))
		menu_backtitle[sizeof(menu_backtitle) - 1] = '\0';
...
static void search_conf (void)
{
	...
	int dres;
	again: dialog_clear ();
	dres = dialog_inputbox (_("Search Configuration Parameter"), _("Enter (sub)string to search for "), 10, 75, "");
	switch (dres) {
	case 0:
		break;
	case 1:
	...
	if (strncasecmp (dialog_input_result, "MCONF_", 7) == 0)
		dialog_input += 7;
...
static void show_help (struct menu *menu)
{
	...
	if (menu_has_help (menu)) {
		if (sym->name) {
			str_printf (&help, "%s%s:\n\n", cmdarg->prefix_len > 0 ? cmdarg->prefix : "", sym->name);
...
/*!
 * @ let the user input a path & write the config file there.
 */
static void conf_save (void)
{
	while (1) {
...
static cmdarg_t *cmdarg_parse (int argc, char **argv)
{
	...
	do {
		optret = getopt_long (argc, argv, "hdp:t:i:T:o:M:H:S:", opts_sa, &optindex);
...
int main (int ac, char **av)
{
	...
	conf_parse (cmdarg->path_in_template); // read template
	conf_read (cmdarg->path_in_config); // read config input
	...
	if (init_dialog (NULL)) {
		fprintf (stderr, N_("Your display is too small to run mconf!\n"));
		fprintf (stderr, N_("It must be at least 19 lines by 80 columns.\n"));
		return 1;
	}

	if (cmdarg->set_default <= 0) { // save default configuration directly
		set_config_filename (conf_get_configname ());
		do {
			conf (&rootmenu); // display ncurses menu
			dialog_clear ();
	//		if (conf_get_changed ())
	//			res = dialog_yesno (NULL, _("Do you wish to save your new configuration?\n<ESC><ESC> to continue"), 6, 60);
	//		else
	//			res = -1;
			res = dialog_yesno (NULL, _("Do you wish to save your new configuration?\n<ESC><ESC> to continue"), 6, 60);
		} while (res == KEY_ESC);
	} else {
		res = 0; // write configuration file by force
	}

	end_dialog (saved_x, saved_y);

	switch (res) {
	case 0: // update output files
